...
function display_img(img){
	...
	if(per==0){
		btn = '<button class="btn btn-secondary text-muted " disabled value="'+img+'"  ><i class="fa fa-trash"></i> </button> '; 
	}else{
		btn = '<button class="btn btn-secondary text-muted" value="'+img+'" onclick="elimina_foto_set(this.value)" ><i class="fa fa-trash"></i> </button> '; 
	}
	...
	var image = '<div class="col-lg-12 cover_image zoom"> </div> \
					...
					<img src="assets/images/sets/'+img+'" class="result_image_set" style="max-height:400px; max-width:95%;  ">'; 
	...
	if(per_edita==0){
		...
		btn_save = '<button class="btn btn-secondary " value="" disabled ><i class="fa fa-save"></i> </button>';  
	}else{
		document.getElementById("foto_name_edit").disabled = false;
		document.getElementById('foto_name_edit').value = img;
		btn_save = '<button class="btn btn-secondary " value="" onclick="toggle(\'\')" ><i class="fa fa-save"></i> </button>'; 
	}	
...
function search_table(table) {
  ...
  input = document.getElementById("campo_buscar");
...
//	var ultimo_c = set_custom.slice(-1);

//	if(ultimo_c=="\n"){

//		var leng = set_custom.length-1;
//		set_custom= set_custom.substring(0,leng);
//	}

//	document.getElementById('sets_custom').value = set_custom + "\n";
//---------------------------------------//////////

/*	var data_sets_b = set_custom.split("\n");
		// a) Analisa cada registro en busqueda de saltos de linea

		for(k=0; k< data_sets_b.length;k++){

			
			//set_cont += data_sets_b[k];

			if(data_sets_b[k]!= "\n"){
				set_cont = set_cont.toString() + data_sets_b[k].toString();
			}
	...
	var num = i+1;
		...
		<card class="col-sm-4 stretch-card " id="set-'+clave+'-'+i+'" searchable="'+clave+'"  style=" width:40%; margin-bottom:2px; box-shadow:none; " > \
			<div class="card shelf_card" style="border: 1px solid rgba(200,200,200,0.9); border-radius:5px; background: #fff; " > \
				...
				\
					<span style="font-size: 9.5px; inline-height: 11px;"> WWW:BRICKSIDEMX:COM </span> <span style="font-size: 9.5px; inline-height:9px;" id="nombre_set_barcode" > | <b>'+tema_code+'</b></span> <br><br> \
					<span style="font-size: 1.2rem; inline-height:1.5rem; " class="barcode_set" id="barcode_set">'+codigo_code+'</span> <br>\
					<span style="font-size: 9.5px; inline-height: 9px;" id="nombre_set_barcode" >'+nombre_code+'</span> \
					<span style="font-size: 9.5px; inline-height: 9px;" id="nombre_set_barcode" >('+num+') </span> \
...
if(data_items!= ''){
	...
	function sidemenu(id){
		...
		var menus = document.getElementsByClassName('side_menu');
		...
		for(i=1;i<=menus.length;i++){
			...
			document.getElementById('side_div_'+i).style.display='none';
			document.getElementById('sidemenu_'+i).style.background='rgba(163, 183, 196, 0.1)'; 
		...
		document.getElementById('side_div_'+data[1]).style.display='block';
		document.getElementById(id).style.background='rgba(163, 183, 196, 0.4)'; 
...
function gpo_opt(id_gpo){
    ...
    var edo_div = document.getElementById('edita_grupo').style.display; 

    if(edo_div=='none'){
        document.getElementById('edita_grupo').style.display = 'block'; 
        
    }else{
        document.getElementById('edita_grupo').style.display = 'none';
...
function actualiza_barra(estado){
...
// <input type="hidden" id="colorb_serie" value="242,215,120">
   ...
   var tot_serie = getElementById('total_serie').value;
   ...
   if(estado == 1){
   		current_nvo = parseInt(cur_serie + 1);
   }else if(estado == 0) {
   		current_nvo = parseInt(cur_serie - 1);
   }
