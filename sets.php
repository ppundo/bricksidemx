		...
		$tot_sets = 1;
                        while($rowb= $result->fetch_assoc()){
	                      ...
	                      // var_dump($data);                        
                            ...
                            $barcode = '<span class="barcode_thin" style="font-size: 1rem; line-height: 0.5rem;">'.$codigob.'</span>'; 

							if($rowb['cantidad']==1){
								$cant = ''; 
							}else{
								$cant = '<i class="fa-solid fa-'.$rowb['cantidad'].'"></i> sets';
							}
		                    ...
		                    if($user_perfil==1){
								...
								$admin_tools = '
								<div class="col-sm-12 ico_set " style="font-size:12px; color: rgba('.$txt_color.',0.8);"   onclick="getSetInfo(\''.$rowb['id'].'\') " > 
									<i class="fas fa-pencil-alt" ></i>
								</div>
								
								<div class="col-sm-12 ico_set " style="font-size:12px; color: rgba('.$txt_color.',0.8);"  onclick="elimina_set(\''.$rowb['id'].'\') " > 
									<i class="fa fa-trash-alt" ></i>
								</div>

								';

								$permiso_elimina = get_permiso_config('26', $perfil_user);

								if($permiso_elimina==1){
									

                                    $btn_elimina_modal ='
									<button type="button"  class="btn btn-inverse-primary" onclick="elimina_set(\''.$rowb['id'].'\'); " > 
									...
									';
                                     ...
                                     $btn_elimina_modal =' <button type="button" disabled class="btn btn-secondary"> <i class="fas fa-trash"></i></button>';
								}
		                   ...
		                   $largo_title = strlen($rowb['nombre']);
							...
							if($rowb['estado']==0){
								...
								*/
								...
								$foto = valida_foto_tipo_adv($rowb['cve_lego'],1);
								...
								$ico_precio = '<div class="col-sm-12 ico_set " style="font-size:12px; '.$style_ico_precio.' color: rgba('.$txt_color.',0.8); " title="'.$disp_precio.'"> <i class="fas fa-dollar-sign"></i></div>';
								
								///
								
								$ico_fecha = '
								<div class="col-sm-12 ico_set " style="font-size:12px; color: rgba('.$txt_color.',0.8)" title="Fecha de Registro: '.formatFecha($rowb['fecha_add']).'"> 
								...
								$list_sets .= '
								<div class="col-sm-12 p-1 border-bottom text-muted" style="text-align:left; " > 
								<span class=" text-sm" style="" >'.$num.'. '.$rowb['nombre'].' </span> 
								</div>';
								...
								$ico_minifig = '
								<div class="col-sm-12 ico_set " style="font-size:12px; opacity: 0.3; color: rgba('.$txt_color.',0.8); " title=" No se han registrado Minifiguras en este set."> 
								...
								$ico_minifig = '
								<div class="col-sm-12 ico_set " style="font-size:12px; color: rgba('.$txt_color.',0.8); " title="Minifiguras Incluidas: '.$rowb['no_minifig'].'"> 
								...
								if($rowb['set_edo']!=1){
                                   ...
                                   $info_edo_set = get_info_edo_set($rowb['set_edo']);
                                    $data_set = explode('|',$info_edo_set);
                                    $id = $data_set[0];
                                    $nombre = $data_set[1];
                                    $icono = '<i class="fa-solid '.$data_set[2].'"></i>';
                                    

									$edo_col = 'incompleto';
									$lbl_edo_coll = '<span class="text-light" style="top: 5px; display:block; position:absolute; botton:0px; right:0px; z-index:9999; margin-right:15%; text-align:right; font-size:0.6rem; padding:3px 10px; background: rgb(89,29,29); background: linear-gradient(90deg, rgba(219,64,64,1) 0%, rgba(89,29,29,1) 98%);" > '.$icono.' '.strtoupper($nombre).'</span>'; 
                                    
                                    $ico_edo_col = '<span class="" title="'.strtoupper($nombre).'" style="padding:0px 3px; font-size:0.8rem; color: rgba('.$txt_color.',0.7); "  > '.$icono.' </span>'; 
								}else{
									$edo_col = 'completo';
									$lbl_edo_coll = '<span class="" style="font-size:0.6rem; padding:3px 10px; color:rgba(150,150,150,0.0); background:rgba(150,150,150,0.0);" > '.$edo_col.'</span>'; 
									//$lbl_edo_col = '';
                                    $ico_edo_col = ''; 
								}
                                    ...
                                    $info_pres = get_info_presentacion($rowb['id_presentacion']);
                                     $data_pres = explode('|',$info_pres);
                                     $id = $data_pres[0];
                                     $nombre = $data_pres[1];
                                     $icono = '<i class="fa-solid '.$data_pres[2].'"></i>';
                                     
                                     $ico_edo_pres= '<span class="" title="'.strtoupper($nombre).'" style="padding: 0px 3px; font-size:0.8rem; color: rgba('.$txt_color.',0.7)" > '.$icono.' </span>'; 
								...
								$lbl_bar = genera_barcode_label_clavelego($rowb['cve_lego'],6,$user_id);
								//$lbl_bar .= genera_barcode_label_clavelego($rowb['cve_lego'],5,$user_id);
								//$select_grupo = genera_select_grupo_inventario($user_id,$rowb['grupo']);
								

								$url_ext = $path_site.'public.php?mnu=7ebec37c9512e433ed508847c24ba1b1&item='.$codigob;
								$input_url_ext = '<input type="hidden" id="url_ext_set_'.$rowb['id'].'" value="'.$url_ext.'"  >'; 

								genera_qrcode_public_set($url_ext,$codigob);
								 
								$search = ''.$rowb['nombre'].' '.$codigob.' '.$rowb['cve_lego'].' '.$edo_col;							
								...
								<card class="col-md-'.$ancho_card.' grid-margin stretch-card " id="set-'.$rowb['id'].'" searchable="'.$search.'" >
									...
									<div class="card shelf_card" style="border: 1px solid rgba(200,200,200,0.9); border-radius:5px; background: #fff; " >
										...
										<div class="set_hover_imagen  " > 
											<img src="data:image/png;base64,'.base64_encode(file_get_contents($foto)).'" id="img_set_'.$rowb['id'].'" style=" position: absolute; max-heigth: 80%; max-width:75%; height:80%; width:auto; right: 18%; bottom: 8px;" >
										</div>
										...
										<div class="hold_icons_set" style="z-index:8002; background:linear-gradient(360deg, rgba('.$color.',0.7) 30%, rgba('.$color.',0) 90%); border-left: 1px solid rgba(255,255,255,0.2); overflow-y:auto; overflow-x: hidden;" >
											...
											<span  style="position: absolute; bottom: 1px; right: 5px; display: block; font-size: 11px; opacity:50%; color: rgba('.$txt_color.',0.8); "> '.$anio.' </span>
												
						                </div> 

										
						                
										<div class="card-body card_body_main " style=" z-index: 8000;   " >

										'.$lbl_edo_col.' 

                                        <div class="hold_icons" style="position:absolute; bottom:3px; background:rgba('.$color.',0.6); padding:1px 8px 1px 5px; border-radius: 5px 0 0 5px; right:40px; border:1px solid rgba('.$color.',0.7); "> 
                                            <div class="icons">
                                                <div class="btn-group" role="group" aria-label="" style="border:0px solid #ccc; z-index:99999999; " >
                                                    '.$ico_edo_pres.' '.$ico_edo_col.'
                                                </div>
                                            </div>
                                        </div>
												...
												<div class="set_hold_info" style="z-index:99;  ">
													...
													<span class=" text-light text-sm" style="margin-top:3px; text-align: center; display:block; filter:opacity:30%; color: rgba('.$txt_color.',0.9);">'.$num.'</span>
													</div>
													...
													<div class="col-md-12 set_data_row set_detail_font" style="font-weight: 300;  " >
														<span class="set_info set_txt_sm" style="color: rgba('.$txt_color.',0.9);" >'.$rowb['cve_lego'].' - '.$rowb['piezas'].' pzs. </span>
													</div>
												
													<div class="col-md-12 set_data_row set_title_font" style="border-bottom:0px solid #ccc;" >
														<span class=" set_info " style="font-size: '.$fontsize.'; '.$line_h.' color: rgba('.$txt_color.',0.9);" > 
														'.$rowb['nombre'].' '.$largo_titl.'
														</span>
													</div>
		
													<div class="set_data_row set_additional_info" >
													'.$cant.'
													</div>
								...
								<div class="col-md-'.$ancho_card.' grid-margin stretch-card " id="lbl_'.$rowb['id'].'" style="display:none" >
								'.$lbl_bar.'
								</div>

								<div id="select_'.$rowb['id'].'" style="display:none">
								'.$select_grupo .$input_url_ext.'
								</div>
...
<card class="col-sm-4 grid-margin stretch-card " id="stats-'.$rowb['cve_lego'].'"  >
	<div class="card shelf_card" style="border: 1px solid rgba(200,200,200,0.9); border-radius:5px; background: #fff; background: rgba(250,250,250,0.3);" >
		...
		<div class="set_back_color" id="" style="background:linear-gradient(230deg, rgba('.$color.',0.6) 40%, rgba('.$s_colorb.',0.8) 120%); height:'.$h.'%; " > </div>
										
		<div class="col-sm-12 set_hold_info" style="width:99%; z-index:100; border:0px solid #fff; ">
										
															
					<div class="col-sm-4 set_data_row set_detail_font hold_icons_set_stats_flat " style="background:linear-gradient(120deg, rgba(130,130,130,0.0) 30%, rgba('.$color.',0.9) 90%); border:0px solid #ccc; z-index:8002"  >
					
							<div class="col-sm-12  set_stat " style="color: rgba('.$txt_color.',0.8);"  > 
								<span  >
									'.$tot_sets.' <span class="col-sm-2 bubble_text"><i class="fa fa-box-open"></i> </span> 
								</span>
							</div>	
...
';     
	...
	//////////                
							...
							$num = $num + 1;
							$tot_sets = $tot_sets + $rowb['cantidad'];
	                    }// while
	    ...
	    $num = 1; 
                        ...
                        while($row= $result->fetch_assoc()){
                            ...
                            for($i=0;$i<=count($data_set);$i++){
                                ...
                                if($data_set[$i]!=0){
                                    $codes .=  ' '.genera_barcode_codigo($data_set[$i],0,$user_id).' ';
                                    $cve_sets .= ' '.$data_set[$i].' ';
                                }
                            ...
                            //$codes = $sets_col;
                            // $codigob = genera_barcode_codigo($rowb['cve_lego'],0,$user_id); $user_id
                            ...
                            if($precio == 0){
                                ...
                                $precio_show = '<i class="fa-solid fa-dollar-sign"></i> '.money_format('%i',$precio);
                            }

	                        $total_tema =$total_tema + $datac[0];
	                        $piezas_tema = $piezas_tema + $datac[1];
	                        $precio_tema = $precio_tema + $datac[2];	                        
				...
				$nombre_tema = $row['nombre'];
				$txt_color = $row['color_txt'];
	                        ...
	                        if($total>=1){
		                        ...
		                        if($total==1){
		                        	...
		                        	$button_label = '
												<a href="index.php?mnu='.$mnu.'&ref='.$row['id'].'&cve=2&thema='.$row['id'].'" class="btn " style="color: rgba('.$txt_color.',0.8); background:linear-gradient(155deg, rgba('.$color.',0.8) 50%, rgba('.$colorb.',0.6) 100%); border:1px solid rgba(250,250,250,0.7);">
													<span id=""  >  Ver set </span> 
												</a>';
		                        }else{                        

		                        	$button_label = '
		                        				<a href="index.php?mnu='.$mnu.'&ref='.$row['id'].'&cve=2&thema='.$row['id'].'"  class="btn " style=" color: rgba('.$txt_color.',0.8); background:linear-gradient(155deg, rgba('.$color.',0.8) 50%, rgba('.$colorb.',0.6) 100%); border:1px solid rgba(250,250,250,0.7);">
													<span id=""  >  Ver sets </span> 
												</a>
												';
		                        ...
		                        $button_label = '
					                        	<a href="#" onclick="toggle(\'nuevo_set\');" class="btn  " style=" color: rgba('.$txt_color.',0.8); background:linear-gradient(155deg, rgba('.$color.',0.8) 50%, rgba('.$colorb.',0.6) 100%); border:1px solid rgba(250,250,250,0.7);"> 
						...
						if($ancho>100){
                            ...
                            $atr = ' height: 75%; max-width: 100%; ';
                        }
								
								$search = ' '.strtolower($row['nombre']).' '.$total.' '.$codes.' '.$cve_sets;
 						
								$len_title=0;
                                
                                $len_title = trim($row['nombre']);
                                $len_title = strlen($row['nombre']);

                                if($len_title < 18){
                                    $title_tema = strtoupper($row['nombre']).' <br> &nbsp; ';
                                }else{
                                    $title_tema = strtoupper($row['nombre']);
                                }
								...
								<card class="col-md-'.$tamanio_card.' grid-margin stretch-card " id="set-'.$row['id'].'" searchable="'.$search.'" style="'.$view_card.'">
									<div class="card shelf_card" style="border: 0px solid rgba(200,200,200,0.9); border-radius:5px; background: #ddd; ">
										...
										<div class="back_color_main fondo_negro_tarjeta" style="border:0px solid #ce0 " id=""></div>
                                        
                                        <div class="col-lg-12 center"  style="z-index:99; position:absolute; bottom:60px;  ">
                                            <img class="" style="opacity:0.5; max-width:80%; max-height:40%; margin-top:20px; '.$atr.' " src="data:image/png;base64,'.base64_encode(file_get_contents('assets/images/logos/'.$foto)).'">																	
                                        </div>	

										<div class="back_color_main set_card_bg" id="" style="background:linear-gradient(155deg, rgba('.$color.',0.8) 50%, rgba('.$colorb.',0.6) 100%); height:'.$h.'%; z-index:100;">
										</div>
						
					                

						
										<div class="card_body center " style="margin-left: 0px; z-index:800; padding-bottom:5px; ">
                                                                                   
                                            <div class="set_card_body center " style="border: 0px solid #ca0;">

                                                <h4 class="set_card_title " style="font-size:1.6rem; clear:both; padding: 2px 10px; border-radius:5px; color: rgba('.$txt_color.',0.7);"> <b>'.$title_tema.' </b></h4>
                                                <span class="bubble_text " style="color: rgba('.$txt_color.',0.6);" ><i class="fa fa-box"></i> '.$total.' sets</span>
                                                <span class="bubble_text " style="color: rgba('.$txt_color.',0.6);" >|</span>
                                                <span class="bubble_text " style="color: rgba('.$txt_color.',0.6);" ><i class="fa fa-puzzle-piece"></i> '.$piezas.' piezas</span><br>
                                                <span class="bubble_text " style="color: rgba('.$txt_color.',0.6);" > '.$precio_show.' </span>
...
$select_origen = '<select class="form-control" id="nvo_origen">
<option value="0" selected > Elije... </option>'.$select_origen.'</select>';
$list_edo_c = genera_select_edo_col(1);
...
<div id="nuevo_set" class="hold_set_edita" style="display: none;  ">
	...
	<div class="col-lg-12 body_modal semi-transparent" style="width: 70%; padding:0px; height:460px;">

            <h5 class="col-md-12 p-2 title_sec_sm bg-head " style="padding:0px; border-radius:5px 0 0 0;"> 
            
                    <i class="fa-solid fa-sliders"></i> <b> Agregar Set </b>
            
                    <div class="btn-group" role="group" style="float:right; border:none;"> 
                        <button type="button" onclick="toggle('nuevo_set')" class=" btn btn-outline-primary  text-clear" style="border:0px solid #ccc;"> 
				...
				<div class="p-1  border-bottom  " style="padding: 10px 0 0 5px;">					
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div id="new_set_status" class="col-md-12" style=" border-radius: 5px;"></div>
                        </div>

                        <div class="col-md-6">
                            <div class=" btn-group " role="group" aria-label="" style="float:right; " >                                                
                                <button type="button" class="btn btn-inverse-primary" onclick="save_new_set();" > <i class="fas fa-save"></i></button>                                      
                            </div>
                        </div>
		...
		<div class="content-wrapper-thin" style="background: #fff; ">
			<div class="col-lg-12">
                ...
                <span class="p1 text-primary grid-margin" style="font-size:0.8rem; margin-bottom: 10px; padding-bottom: 10px;">* Campos Requeridos</span>
				...
				<div class="row">
					...
					<div class="col-md-6 grid-margin ">
						<form name="form_up_set" method="post" action="?" id="form_up_set" enctype="multipart/form-data">
							<div class="form-group row compacted">
								<label for="edita_index" class="col-sm-3 col-form-label">Clave<span class="text-success">*</span></label>
								<div class="col-sm-8">
							...
							<div class="form-group row compacted">
								<label for="edita_id" class="col-sm-3 col-form-label">Nombre<span class="text-success">*</span></label>
								<div class="col-sm-8">
							...
							<div class="form-group row compacted">
								<label for="edita_id" class="col-sm-3 col-form-label">Cantidad</label>
								<div class="col-sm-8">
									<input type="number" min="1" class="form-control"  id="new_set_cantidad" value="1" placeholder="Cantidad">
								</div>
							</div>
							<div class="form-group row compacted">
								<label for="edita_edo_col" class="col-sm-3 col-form-label">Estado Coleccion</label>
								<div class="col-sm-8">
									<select id="new_estado_col" class="form-control">
							...
							<div class="form-group row compacted">
								<label for="edita_serie" class="col-sm-3 col-form-label">Tema<span class="text-success">*</span></label>
								<div class="col-sm-8">
					...
					<div class="col-md-6 grid-margin " style="border: 0px solid #c40;">
                        ...
                        <div class="form-group row compacted ">
							<label for="edita_sku" class="col-sm-3 col-form-label">Codigo de Barras Caja</label>
							<div class="col-sm-8">
								<input type="text" min="0" class="form-control" id="new_set_codebar_box" placeholder="Codigo de barras caja">
							</div>
						</div>
...
<div id="edit_set" class="hold_set_edita" style="display:none;">
    ...
    <div class="col-lg-12 body_modal semi-transparent border" style="width: 90%; padding:0px;">

            <h5 class="col-md-12 p-2 title_sec_sm bg-head " style="padding:0px; border-radius:5px 0 0 0;"> 
            
                    <i class="fa-solid fa-sliders"></i> <b> Edita Set </b>
            
                    <div class="btn-group" role="group" style="float:right; border:none;"> 
                        <button type="button" onclick="toggle('edit_set')" class="btn btn-outline-primary text-clear" style="border:0px solid #ccc;"> 
                            <i class="fa-solid fa-times fa-lg"></i>
                        </button>
                    </div> 
             </h5> 

<!-- Barra de opciones y estado -->
				<div class="p-1  border-bottom  " style="padding: 10px 0 0 5px;">					

                    <div class="row">
                        <div class="col-md-6">
                            <div id="set_status" class="col-md-12" style=" border-radius: 5px;"></div>
                        </div>

                        <div class="col-md-6">

                                <div class="btn-group " role="group" aria-label="" style="margin-left: 25px; float:right; " >					
                                    
                                    <button type="button" class="btn btn-inverse-primary" onclick="save_set();" > 
                                        <i class="fas fa-save"></i> 
                                    </button>
                                    
                                    <?php echo $btn_elimina_modal ?>	                                        
                                    <?php echo $btn_show;  ?>
                                </div>  
        ...
        <div class="content-wrapper-thin" style="background:#fff; ">
            ...
            <div class=" col-lg-12">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 " style="border: 0px solid #d30; height:300px; overflow:scroll; overflow-y: auto; overflow-x: hidden;">
                            ...
                            <div class="row compacted ">
                                ...
                                <div class="col-sm-8">
                                    <input type="text"  disabled="disabled" class="form-control" name="set_cve" id="set_cve" placeholder="Clave Set">
                                </div>
                            </div>
                            ...
                            <div class="row compacted ">
                                ...
                                <div class="col-sm-8">
                                    <select class="form-control" id="set_tema"></select>
                                </div>
                            </div>

							<div class="row compacted ">
                                <label for="edita_serie" class="col-sm-4 col-form-label lbl_form_set">Edo Col</label>
                                <div class="col-sm-8">
                                    <select class="form-control" id="set_edo_col"></select>
                                </div>
                            </div>

                            <div class="row compacted ">
                                <label for="edita_serie" class="col-sm-4 col-form-label lbl_form_set">Presentacion</label>
                                <div class="col-sm-8">
                                    <select class="form-control" id="set_presentacion"></select>
                                </div>
                            </div>

                            <div class=" row compacted ">
                                <label for="edita_sku" class="col-sm-4 col-form-label lbl_form_set">Origen</label>
                                <div class="col-sm-8" id="hold_origen"> </div>
                            </div>
                            ...
                            <div class=" row compacted ">
                                <label for="edita_index" class="col-sm-4 col-form-label lbl_form_set">Código Caja</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="set_codebar_box" placeholder="Codigo Caja Original">
                                    <input type="hidden" id="set_id">
                                </div>
                            </div>

							<div class=" row compacted ">
                                <label for="edita_sku" class="col-sm-4 col-form-label lbl_form_set">Cantidad:</label>
                                <div class="col-sm-8">
                                    <input type="number" min="1" class="form-control" id="set_cantidad" placeholder="¿Cuantos sets?">
                                </div>
                            </div>
                    ...
                    <div class="col-md-2 border-left border-right bg-light" style="padding: 0px; max-height: 298px; margin-left: 10px;">
                        ...
                        <div class="row col-md-12  bg-head sidemenu text-muted " id="sidemenu_1" style="" onclick="sidemenu(this.id)" title="Conf. Gral. Colección">
                            <span class="col-1 " style=" font-size:1.1em; "><i class="fa fa-fas fa-image"></i></span>
                            <span class="col-6 " style="font-size:1.1em;" onclick="sidemenu(this.id)"> Imágen</span>
                        ...
                        <div class="row col-md-12  bg-head-low  sidemenu text-muted" id="sidemenu_2" style="" onclick="sidemenu(this.id)" title="Etiqueta">
                            <span class="col-1 " style=" font-size:1.1em; "><i class="fa-solid fa-box-open"></i></span>
                            <span class="col-6 " style="font-size:1.1em;" onclick="sidemenu(this.id)"> Inventario</span>
                            <span class=" col-1 text-head" style="float:right; right:-15px; font-size:1em; " onclick="sidemenu(this.id)"><i class="fa-solid fa-chevron-right"></i></span>
                        </div>

						<div class="row col-md-12  bg-head-low  sidemenu text-muted" id="sidemenu_3" style="" onclick="sidemenu(this.id)" title="Etiqueta">
                            <span class="col-1 " style=" font-size:1.1em; "><i class="fa-solid fa-square-arrow-up-right"></i></span>
                            <span class="col-6 " style="font-size:1.1em;" onclick="sidemenu(this.id)"> Público</span>
                            <span class=" col-1 text-head" style="float:right; right:-15px; font-size:1em; " onclick="sidemenu(this.id)"><i class="fa-solid fa-chevron-right"></i></span>
                        </div>
                    ...
                    <div class="col-md grid-margin  center " style="text-align:center; height: 298px; max-height: 298px;">
					
					<div class="" id="div_sidemenu_0" style="display: block; margin-bottom:10px;"> 
						<h5 id="set_title" class="col-md-12 p-2 title_sec bg-theme" style="font-size: 1rem; font-weight:400; padding: 3px 15px; "></h5>
					</div>

						<div class="" id="div_sidemenu_1" style="display: block;"> 
							
                            
                            <div class="col-md-12 form-group compacted" id="display_image_set" style=" margin-top:10px; display: block; border: 0px solid #ccc; max-height: 250px; "> </div>
                        </div>


                        <div class="" id="div_sidemenu_2" style="display: none;">
                            <h5 class="col-md-12 p-2 no-show  subtitle_sec"> Inventario  </h5>

                            <div class="row">
                                <div class="col-sm-6" style="text-align: left;">
                                    
								<div class="row compacted">
									<label for="edita_id" class="col-sm-4 col-form-label lbl_form_set">Grupo</label>
                                    <div class="col-sm-8" >
										<input type="text" disabled="disabled" class="form-control" id="select_grupo" placeholder="Grupo">
                                    </div>
								</div>

								<div class="row compacted">
                                    <label for="edita_id" class="col-sm-4 col-form-label lbl_form_set">Ubicación</label>
                                    <div class="col-sm-8" >
										<input type="text" disabled="disabled" class="form-control" id="select_ubic" placeholder="Ubicacion">
                                    </div>
								</div>
                                ...
                                <div class="col-sm-6">
                                    <form>
                                        <div class="border" id="lbl_barcode">

                                            <input type="hidden" name="flatRadios" value="2">
                                        </div>
                                        <br>
						...
						<div class="" id="div_sidemenu_3" style="display: none;">
                            <h5 class="col-md-12 p-2 no-show subtitle_sec"> Vista al Público </h5>
							
							<div class="row">
								
                                <div class="col-sm-4 " style="text-align: center;" id="qr_ext_link"></div>
								
                                <div class="col-sm-8" style="text-align: left; padding:0px;  padding-top:10px;" >
									
                                    
                                    <div class="form-group col-md-10" style="padding:0px;">
			                          <div class="input-group">
			                            <div class="input-group-prepend bg-muted border-primary">
			                              <span class=" input-group-text bg-transparent text-muted" style="">
			                                URL
			                              </span>
			                            </div>
			                            <input type="text" class="form-control " id="set_url_ext" placeholder="URL" aria-label="URL" aria-describedby="">
			                          </div>
			                        </div>

									<div class="" id="url_ext_link" style="margin-top: 20px; text-align: left;" >test</div>

                                    <span class="text-muted" style="text-align: left; font-size:0.7rem;" id="url_ext_btn"></span>
								</div>
...
<div id="lista_set" class="hold_set_edita"  style="<?php echo $list_vis ?> "  >
    ...
    <div class="col-lg-12 body_modal semi-transparent" style="width: 30%; padding:0px;">


    <h5 class="col-md-12 p-2 title_sec_sm bg-head " style="padding:0px; border-radius:5px 0 0 0;"> 
            
            <i class="fa-solid fa-sliders"></i> <b> Lista de Sets </b>
